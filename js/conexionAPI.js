async function listarProductos(){
    const conexion = await fetch("/Ruluda-Store/json/db.json",{
    method:"GET",
    headers:{
...
async function buscarProducto(palabraClave){
    ...
    try {const conexion = await fetch(`/Ruluda-Store/json/db.json?q=${palabraClave}`);
    const conexionConvertida = await conexion.json();
...
async function eliminarProducto(id) {
    const conexion = await fetch(`/Ruluda-Store/json/db.json/${id}`, {
        method: "DELETE",
        headers: {
            "Content-type": "application/json"
        }
